...
agent_communication:
  ...
  - agent: "main"
    message: "🔥 FACEBOOK GRAPH API INTEGRATION - TESTING PHASE: Facebook Access Token configured in .env (718756950794070|EybKV2tc5c9qQQZiwQdpDWf0gnA). Backend service restarted successfully. Ready to test Facebook API connectivity through existing mapa-territorial endpoint and then add dedicated Facebook endpoints if needed. Current integration uses Facebook data in weighted calculations (Facebook: 35%, Instagram: 40%, Twitter: 25%) for territorial analysis. About to test real Facebook API connectivity."
